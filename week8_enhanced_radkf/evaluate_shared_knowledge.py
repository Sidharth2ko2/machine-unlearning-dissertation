"""
Shared-knowledge evaluation for Week 8 enhanced RA-DKF experiments.
...
from enhanced_ra_dkf import get_features
...
@torch.no_grad()
def limited_accuracy(model, loader, device, max_samples):
    model.eval()
    correct = total = 0
    for x, y in loader:
        remaining = max_samples - total
...
@torch.no_grad()
def limited_per_sample_loss(model, loader, device, max_samples):
    model.eval()
...
def evaluate_model_limited(model, loaders, device, max_samples):
    """Fast standard metrics for smoke trials; use full evaluation for final claims."""
    acc_dr = limited_accuracy(model, loaders["retain"], device, max_samples)
    acc_df = limited_accuracy(model, loaders["forget"], device, max_samples)
    acc_val = limited_accuracy(model, loaders["test"], device, max_samples)

    forget_losses = limited_per_sample_loss(model, loaders["forget"], device, max_samples)
    test_losses = limited_per_sample_loss(model, loaders["forget_class_test"], device, max_samples)
    ...
    if n < 10:
        ...
        clf.fit(x_mia, y_mia)
        mia = accuracy_score(y_mia, clf.predict(x_mia)) * 100.0
...
def evaluate_shared_knowledge(
    ...
    if standard_max_samples is None:
        standard = evaluate_model(student, loaders, device)
    else:
        standard = evaluate_model_limited(student, loaders, device, standard_max_samples)
    out = dict(standard)
